      ...
      <DialogContent className="w-full max-w-2xl max-h-[90vh] overflow-y-auto overflow-x-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            <h3 className="font-semibold text-lg text-blue-800 mb-2">Room Information</h3>
            <div className="grid grid-cols-2 gap-2 min-w-0">
              {booking.rooms.map((room, index) => (
                <div key={index} className="bg-white p-3 rounded-md shadow-sm border">
                  <p className="break-words"><strong>Type:</strong> {room.roomType}</p>
                  <p><strong>Capacity:</strong> {room.roomId.capacity} persons</p>
                  <p><strong>Quantity:</strong> {room.quantity}</p>
                  <p><strong>Price per Night:</strong> ₹{room.roomId.price}</p>
                  <p><strong>Number of Nights:</strong> {room.numberOfNights}</p>
                </div>
              ))}
            </div>
          </div>
