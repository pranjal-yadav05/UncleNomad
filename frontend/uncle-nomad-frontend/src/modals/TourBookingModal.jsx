...
export default function TourBookingModal({
  ...
  const handleBookingSubmit = async (e) => {
    ...
    if (!isOtpVerified) {
      setValidationErrors((prev) => ({
        ...prev,
        otp: "Email verification is required before proceeding",
      }));
      setIsLoading(false);
      return;
    }
  ...
  const handlePaymentSuccess = async (paymentResponse) => {
    try {
      ...
      // Format the booking data with all required fields for the ticket
      const formattedBooking = {
        ...updatedBooking,
        tourName: selectedTour.title,
        tourDate:
          updatedBooking.selectedDate?.startDate || selectedTour.startDate,
        tourEndDate:
          updatedBooking.selectedDate?.endDate || selectedTour.endDate,
        duration: selectedTour.duration,
        tourImage: selectedTour.images?.[0] || null,
        pricePerPerson:
          updatedBooking.selectedPackage?.price ||
          updatedBooking.totalPrice / updatedBooking.groupSize,
      ...
      setPaymentData({
        ...paymentData,
        confirmedBooking: formattedBooking,
      });
    ...
    <>
      ...
      <AlertDialog open={isDisclaimerOpen} onOpenChange={setIsDisclaimerOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDescription>
              ...
              <ul className="list-disc pl-5 mt-2 space-y-1">
                ...
                <li>
                  Total amount: ₹
                  {selectedTour?.price * bookingDetails.groupSize}
                </li>
                <li>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={handleCancelDisclaimer}>
              Cancel
            </AlertDialogCancel>
