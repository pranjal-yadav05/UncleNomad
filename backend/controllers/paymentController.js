...
export const initiatePayment = async (req, res) => {
  try {
    ...
    // Validate required fields - make phone required but email optional
    if (!bookingData || !amount || !phone) {
    ...
    // Use phone number for receipt ID instead of email
    const receiptId = `RECEIPT_${timestamp}_${phone
      .toString()
      .replace(/\D/g, "")
      .substring(0, 10)}`;
    ...
    const orderOptions = {
      ...
      notes: {
        ...
        ...(email && { email: email.substring(0, 50) }),
      },
    };
...
export const paymentCallback = async (req, res) => {
  try {
    ...
    if (!isValidChecksum) {
      console.error(
        "Checksum verification failed for order:",
        receivedData.body?.ORDERID
      );
      return res
        .status(400)
        .json({ status: "ERROR", message: "Checksum verification failed" });
    }
    ...
    const updateBooking = async (statusData) => {
      try {
        ...
        try {
          ...
          console.error(
            "Failed to update booking status on retry:",
            retryError
          );
          throw retryError;
        }
    ...
    if (STATUS === "TXN_SUCCESS" && RESPCODE === "01") {
      ...
      return res.redirect(
        `${process.env.FRONTEND_URL}/booking-success/${ORDERID}`
      );
    } else {
      await updateBooking({
      ...
      return res.redirect(
        ...
        }/booking-failed?orderId=${ORDERID}&error=${encodeURIComponent(
          RESPMSG
        )}`
      );
    }
