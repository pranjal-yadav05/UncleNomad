...
async function generateBookingsExcel(bookings) {
  ...
  bookings.forEach((booking) => {
    ...
    const roomsInfo = booking.rooms
      .map((room) => {
        // Handle cases where roomId is null or not populated
        if (!room.roomId) {
          return `Unknown Room x ${room.quantity}`;
        }

        // Check if roomId is an object (populated) or just an ID (not populated)
        const roomName =
          typeof room.roomId === "object" && room.roomId !== null
            ? room.roomId.name || "Unnamed Room"
            : "Unknown Room";
        const roomType =
          typeof room.roomId === "object" && room.roomId !== null
            ? room.roomId.type || "Unknown Type"
            : "Unknown Type";
